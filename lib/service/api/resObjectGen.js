...
Parser.prototype = {
    ...
    _doBaseType: function(baseType) {
        ...
        if(this._isFunction(baseType)) {
            try {
                return baseType.call(null, this._getValueStack(), this.req, 
                    this._getIteratInfo());
            } catch (e) {
                logger.error("{{Error invoking functions: [" + baseType + "]." +
                " Detailed: " + e.message+" }}");
                return "{{Error invoking functions: [" + baseType + "]." +
                " Detailed: " + e.message+" }}";    
            }
        }
     ...
     */
     _getElValue: function(key) {
        var result, ks = key.split("."), i,
            iterationInfo = this._getIteratInfo(),
